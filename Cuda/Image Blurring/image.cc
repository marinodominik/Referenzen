...
image_gpu::image_gpu(int w, int h) : width(w), height(h)
{
	...
	cudaMallocPitch((void**)&this->data, &this->pitch, size * sizeof(int), this->width);
}
...
{	
	std::cout << "image uploaded\n" << std::endl;
	int size = dst.height * dst.width;
	cudaMemcpy(dst.data, this->data, size * sizeof(int), cudaMemcpyHostToDevice);
}

void image_cpu::download(const image_gpu &src)
{	
	std::cout << "image downloaded\n" << std::endl;
	int size = this->height * this->width;
	cudaMemcpy(this->data, src.data, size * sizeof(int), cudaMemcpyDeviceToHost);
}
