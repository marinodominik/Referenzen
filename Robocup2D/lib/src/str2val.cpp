...
int strfind(char const* str,char const* expr, char const* & next) {
  // tmp pointer
  char const* strcopy_p;
  char const* exprcopy_p;
  ...
  while(!str_found) {
    ...
    while( IS_WHITE_CHAR(*str) || *str != *expr )
      str++;
  
    // copy string pointers for the case that the following chars
    // do not match
    exprcopy_p = expr;
    if(!IS_STR_END_CHAR(*str))
  	  strcopy_p = (str+1);
...
int str2val(char const* str,int & val, char const* & next) {
  ...
  if (ERANGE == errno)
  {
    /*cerr<<" str==error_ptr?"<<(str==error_ptr)<<" errno?"<<(errno)
        <<" tmp_long?"<<tmp_long<<" tmp_long>LONG_MAX?"<<(tmp_long>INT_MAX)
        <<" tmp_long<LONG_MIN?"<<(tmp_long<INT_MIN)<<" STR:"<<str<<endl;*/
  }
...
int str2val(char const* str,long & val, char const* & next) {
  ...
  if ( str == error_ptr
    ...
    /*cerr<<" str==error_ptr?"<<(str==error_ptr)<<" errno?"<<(errno)
        <<" tmp_long?"<<tmp_long<<" tmp_long>LONG_MAX?"<<(tmp_long>LONG_MAX)
        <<" tmp_long<LONG_MIN?"<<(tmp_long<LONG_MIN)<<" STR:"<<str<<endl;*/
...
int str2val(char const* str,double & val, char const* & next) {
  ...
  /*if ( str == error_ptr
    ...
    /*cerr<<" str==error_ptr?"<<(str==error_ptr)<<" errno?"<<(errno)
        <<" tmp_double?"<<tmp_double<<" STR:"<<str<<" error_ptr:"<<error_ptr<<endl;*/
