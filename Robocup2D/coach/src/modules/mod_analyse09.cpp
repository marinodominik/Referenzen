...
const double ModAnalyse09::MOVEMENT_EPSILON(0.1);


bool ModAnalyse09::init(int argc,char **argv) {
...
bool ModAnalyse09::destroy() {
  
  return true;
}

Player *ModAnalyse09::getPlByIndex(int i,Field *field) {
  if(i<TEAM_SIZE) {
...
bool ModAnalyse09::behave() 
{
  if (ivRecentTimeBehaveEntered == fld.getTime())
  {
    LOG_FLD(0,"ModAnalyse09: RETURN IMMEDIATELY. Multiple enterings of behave at t="<<ivRecentTimeBehaveEntered);
    return false;
  }
...
LOG_FLD(0,"ModAnalyse09: play mode at t="<<fld.getTime()<<" is "
      ...
      for (int t=0; t<pltypes; t++) 
      {
	      if (fld.myTeam[p].possibleTypes[t] >= 0) 
        {
          ...
          { 
            ...
            {
              LOG_FLD(1,<<"ModAnalyse09: **** I am sure: Own player #"<<p+1<<" is not of type "
                <<t<<"! ("<<fld.myTeam[p].possibleTypes[t]<<" Points)");
              //cout<<"\nModAnalyse09: **** I am sure: Own player #"<<p+1<<" is not of type "
              //  <<t<<"! ("<<fld.myTeam[p].possibleTypes[t]<<" Points)";
          ...
          else
          {
            ...
            {
              LOG_FLD(0,<<"ModAnalyse09: RECONSIDER own player #"<<p+1<<", not of type "<<t<<" ("
                <<(fld.myTeam[p].possibleTypes[t])<<")!");
              //std::cerr << "\nModAnalyse09: RECONSIDER own player #"<<p+1<<", not of type "<<t<<"!";
              //std::cout << "\nModAnalyse09: RECONSIDER own player #"<<p+1<<", not of type "<<t<<"!";
...
showPossTypeTable();
        ...
        if (fld.myTeam[p].possibleTypes[t] == -2)
        {
          if (possTypes[p][t] == 0)
          {
            LOG_FLD(0,<<"ModAnalyse09: CANCEL SPEED PROGRESS ASSUMPTION for own player #"<<p+1<<", eventually really of type "<<t<<"!");
            //std::cerr<<"\nModAnalyse09: CANCEL SPEED PROGRESS ASSUMPTION for own player #"<<p+1<<", eventually really of type "<<t<<"!";
            fld.myTeam[p].possibleTypes[t] = 0;
          }
          else 
          if (possTypes[p][t] > PLAYER_FUZZY_WEIGHT)
          {
            LOG_FLD(0,<<"ModAnalyse09: OVERRIDE FUZZY ASSUMPTION for own player #"<<p+1<<", definitely not of type "<<t<<"!");
            //std::cerr<<"\nModAnalyse09: OVERRIDE FUZZY ASSUMPTION for own player #"<<p+1<<", definitely not of type "<<t<<"!";
            fld.myTeam[p].possibleTypes[t] = -1;
            cnt ++;
          }
          else
      ...
      if ( cnt==pltypes )
      {
        for (int t=0; t<pltypes; t++)
        {
          if ( fld.myTeam[p].possibleTypes[t] == -2 )
          {
            LOG_FLD(0,<<"ModAnalyse09: CANCEL FUZZY ASSUMPTION for own player #"<<p+1<<", eventually really of type "<<t<<"!");
            //std::cerr<<"\nModAnalyse09: CANCEL FUZZY ASSUMPTION for own player #"<<p+1<<", eventually really of type "<<t<<"!";
            fld.myTeam[p].possibleTypes[t] = 0;
          }
          cnt -- ;
        }
      ...
      if ( cnt==pltypes ) 
      {
        LOG_ERR(0,<<"ERROR: ModAnalyse09: Something went really wrong, there is no possible type for "
                  <<" own player #"<<p+1);
        LOG_FLD(0,<<"ERROR: ModAnalyse09: Something went really wrong, there is no possible type for "
                  <<" own player #"<<p+1);
      ...
      if ( cnt==pltypes-1 ) 
      {
        if(!successMentioned[p]) 
        {
          ...
          for (t=0; t<pltypes; t++) 
            if ( fld.myTeam[p].possibleTypes[t] >= 0 ) 
              break;
          LOG_DEF(0,<<"ModAnalyse09: ****** I am sure: My player #"<<p+1
                    <<" must be of type "<<t<<"!");
      ...
      for(int t=0;t<pltypes;t++) 
      {
        if (fld.hisTeam[p].possibleTypes[t]>=0) 
        {
          ...
          if (fld.hisTeam[p].type!=t) 
          {
            ...
            {
              LOG_FLD(1,<<"ModAnalyse09: **** I am sure: Opponent player #"<<p+1<<" is not of type "
          ...
          else
          {
            ...
            {
              LOG_FLD(0,<<"ModAnalyse09: RECONSIDER opp player #"<<p+1<<", not of type "<<t<<" ("
                <<(fld.hisTeam[p].possibleTypes[t])<<")!");
              //std::cerr << "\nModAnalyse09: RECONSIDER opp player #"<<p+1<<", not of type "<<t<<"!";
              //std::cout << "\nModAnalyse09: RECONSIDER opp player #"<<p+1<<", not of type "<<t<<"!";
              ...
              //TG2010: type t can now be considered for other players again
              if (getNumberOfRemainingTypesForPlayer(p+TEAM_SIZE) == 0)
                for (int t2=0; t2<pltypes; t2++)
                   fld.hisTeam[p].possibleTypes[t2] = 0;
              for (int p2=0; p2<TEAM_SIZE; p2++)
                 fld.hisTeam[p2].possibleTypes[t] = 0;
showPossTypeTable();
        ...
        else if (fld.hisTeam[p].possibleTypes[t] == -2)
        {
          if (possTypes[p][t] == 0)
          {
              LOG_FLD(0,<<"ModAnalyse09: CANCEL SPEED PROGRESS ASSUMPTION for opp player #"<<p+1<<", eventually really of type "<<t<<"!");
              //std::cerr<<"\nModAnalyse09: CANCEL SPEED PROGRESS ASSUMPTION for opp player #"<<p+1<<", eventually really of type "<<t<<"!";
              fld.hisTeam[p].possibleTypes[t] = 0;
          }
          else 
          if (possTypes[p][t] > PLAYER_FUZZY_WEIGHT)
          {
            LOG_FLD(0,<<"ModAnalyse09: OVERRIDE FUZZY ASSUMPTION for opp player #"<<p+1<<", definitely not of type "<<t<<"!");
            //std::cerr<<"\nModAnalyse09: OVERRIDE FUZZY ASSUMPTION for opp player #"<<p+1<<", definitely not of type "<<t<<"!";
            fld.hisTeam[p].possibleTypes[t] = -1;
            cnt ++;
          }
          else
      ...
      if ( cnt==pltypes )
      {
        for (int t=0; t<pltypes; t++)
        {
          ...
          {
            LOG_FLD(0,<<"ModAnalyse09: CANCEL FUZZY ASSUMPTION for opp player #"<<p+1<<", eventually really of type "<<t<<"!");
            //std::cerr<<"\nModAnalyse09: CANCEL FUZZY ASSUMPTION for opp player #"<<p+1<<", eventually really of type "<<t<<"!";
            fld.hisTeam[p].possibleTypes[t] = 0;
          }
          cnt -- ;
        }
      ...
      if(cnt==pltypes-1) 
      {
        if (!successMentioned[p+TEAM_SIZE]) 
        {
          ...
          for (t=0;t<pltypes;t++) 
            if(fld.hisTeam[p].possibleTypes[t]>=0) 
              break;
          LOG_DEF(0,<<"ModAnalyse09: ****** I am sure: Opponent player #"
...
showPossTypeTable();
    ...
    if (flg) 
    {
      LOG_DEF(0,<<"ModAnalyse09: Revised table of known player types:");
      LOG_DEF(0,<<"ModAnalyse09:");
      LOG_DEF(0,<<"ModAnalyse09:  Pl | 1 | 2 | 3 | 4 | 5 | 6 | 7 | 8 | 9 | A | B |");
      LOG_DEF(0,<<"ModAnalyse09: -------------------------------------------------");
      if(START_PL<TEAM_SIZE) 
      {
        oss.seekp(0);
        oss << "ModAnalyse09: Own |";
        for (int p=0;p<TEAM_SIZE;p++) 
        {
          ...
          if (onlyOneLeft[p]) 
          {
            int j;
            for (j=0;j<pltypes;j++) 
              if (fld.myTeam[p].possibleTypes[j]>=0) 
                break;
	            oss << " "<<j<<" |";
              continue;
          }
        ...
        LOG_DEF(0,<<oss.str());
        for (int p=0;p<TEAM_SIZE;p++)
          LOG_DEF(0,<<"ModAnalyse09: remaining types for own player "<<p+1<<": "
            <<getNumberOfRemainingTypesForPlayer(p));
      }
      oss.seekp(0);
      oss << "ModAnalyse09: Opp |";
      for(int p=0;p<TEAM_SIZE;p++) 
      {
      ...
      LOG_DEF(0,<<oss.str());
      for (int p=0;p<TEAM_SIZE;p++)
        LOG_DEF(0,<<"ModAnalyse09: remaining types for opp player "<<p+1<<": "
       ...
       && (!changeAnnounced || !goalieAnnounced) ) 
  {
      ...
      if(fld.getTime()==1 || fld.getTime()>=50) 
      {
            ...
            && fld.getTime()-fld.sayInfo.lastInPlayOn[MSGT_INFO]<=ServerParam::clang_win_size) 
        {
               ...
               || fld.getTime()-fld.sayInfo.lastInPlayOn[MSGT_ADVICE]>ServerParam::clang_win_size) 
          {
            ...
            {
              LOG_DEF(0,<<"ModAnalyse09: Send opp types via MSGT_ADVICE at t="
                <<fld.getTime()<<": "<<oss.str());
        ...
        {
          LOG_DEF(0,<<"ModAnalyse09: Send opp types via MSGT_INFO[1] at t="
            <<fld.getTime()<<": "<<oss.str());
    ...
    {
      LOG_DEF(0,<<"ModAnalyse09: Send opp types via MSGT_INFO[2] at t="
        <<fld.getTime()<<": "<<oss.str());
      sendMsg(MSG_SAY,MSGT_INFO,oss.str());
      if(goalie>0) goalieAnnounced=true;
      changeAnnounced=true;
...
void ModAnalyse09::checkCollisions() 
{
...
bool ModAnalyse09::checkKick() 
{
  ...
  if(ballKicked) 
  {
    for(int i=0;i<TEAM_SIZE*2;i++) 
      canKick[i]=false;
    //LOG_FLD(0,<<"ModAnalyse09: Ball has been kicked!");
    for (int i=0;i<TEAM_SIZE*2;i++) 
    {
    ...
    if (!cnt) 
    {
      LOG_FLD(1,<<"ModAnalyse09: Ball has been kicked, but I don't know by whom... Collision?");
    } 
    else 
    {
      if (cnt==1) 
      {
        ...
        {   // ignore in case of possible collision
          LOG_FLD(2,<<"ModAnalyse09: Not sure if player kicked or collided -> ignoring...");
        }
        else 
        {
          ...
          if(kickpl<TEAM_SIZE) 
          {
            LOG_FLD(2,<<"ModAnalyse09: Ball was kicked by own player #"<<kickpl+1);
          } 
          else 
          {
            LOG_FLD(2,<<"ModAnalyse09: Ball was kicked by opponent player #"<<kickpl+1-TEAM_SIZE);
          }
          ...
          for (int t=0;t<pltypes;t++) 
          {
      	    if (kr>kickRanges[t]+.001) 
            {
      	      ...
      	      if (kickpl<TEAM_SIZE) 
              {
                LOG_FLD(2,<<"ModAnalyse09: *** Own player #"<<kickpl+1<<" probably not of type "
                          <<t<<" (kickrange)");
      	      } 
              else 
              {
                LOG_FLD(2,<<"ModAnalyse09: *** Opp player #"<<kickpl-TEAM_SIZE+1
      ...
      {
        LOG_FLD(2,<<"ModAnalyse09: Ball was kicked, but there are several players in kickrange.");
      }
    }
  }
  return true;
...
/** Player Decay testen */
bool ModAnalyse09::checkPlayerDecay() 
{
  ...
  for(int p=START_PL;p<2*TEAM_SIZE;p++) 
  {
    ...
    for(int t=0;t<pltypes;t++) 
    {
      ...
      if(randx>rmax+0.0000001 || randy>rmax+0.0000001) 
      {
        if (p<TEAM_SIZE) 
        {
          //LOG_FLD(2,<<"ModAnalyse09: *** Own player #"<<p+1<<" probably not of type "
          //	  <<t<<" (player_decay)");
          LOG_FLD(2,<<"ModAnalyse09: *** Own player #"<<p+1<<" not type "
                    <<t<<" (player_decay, rmax="<<rmax<<", randx="<<randx
                    <<", randy="<<randy<<")");
        } 
        else
        {
          //LOG_FLD(2,<<"ModAnalyse09: *** Opp player #"<<p+1-TEAM_SIZE<<" probably not of type "
          //  <<t<<" (player_decay)");
          LOG_FLD(2,<<"ModAnalyse09: *** Opp player #"<<p-TEAM_SIZE+1<<" not type "
...
bool ModAnalyse09::checkInertiaMoment() {
  ...
  for(int p=START_PL;p<TEAM_SIZE*2;p++) {
	...
	if(p<TEAM_SIZE) {
	  LOG_FLD(0,<<"ModAnalyse09: *** Own player #"<<p+1<<" probably not of type "
		  <<t<<" (inertia) act="<<actangle<<" max="<<ang);
	} else {
	  LOG_FLD(0,<<"ModAnalyse09: *** Opp player #"<<p-TEAM_SIZE+1<<" probably not of type "
		  <<t<<" (inertia) act="<<actangle<<" max="<<ang);
	}
...
bool ModAnalyse09::checkPlayerSpeedMax() 
{
  Player *player,*oldplayer;
  for(int p=START_PL;p<TEAM_SIZE*2;p++) 
  {
    ...
    for(int t=0;t<pltypes;t++) 
    {
      ...
      if (actspeed>maxspeed+.0001) 
      {
        ...
        {
          LOG_FLD(2,<<"ModAnalyse09: *** Opp player #"<<p-TEAM_SIZE+1<<" probably not of type "
            <<t<<" (player_speed_max)" <<"act="<<actspeed<<" max="<<maxspeed);
        }
...
bool ModAnalyse09::onRefereeMessage(bool PMChange) {
  
  return true;
}

bool ModAnalyse09::onKeyboardInput(const char *str) {

  return false;
}

bool ModAnalyse09::onHearMessage(const char *str) {

  return false;
}

bool ModAnalyse09::onChangePlayerType(bool ownTeam,int unum,int type) {
  if(ownTeam) 
  {
...
//END OF CLASS ModAnalyse09

bool
ModAnalyse09::updateSpeedProgressInformationArray()
{
...
bool
ModAnalyse09::checkSpeedProgress()
{
    ...
    for (int t=0; t<pltypes; t++) 
    {
      ...
      if ( error > ERROR_SHARE_THRESHOLD*(maxError-minError) + minError ) 
      {
        ...
        if ( realType == t )
        {
          ...
          LOG_FLD(0,<<"ModAnalyse09: ==> "<<num);
        }*/
      }
    }
    LOG_FLD(0,<<"ModAnalyse09: pl_"<<p<<" (type="
...
bool
ModAnalyse09::checkMaxSpeedProgress()
{
  //try to exploit the information gathered
  for (int p=START_PL; p < 2*TEAM_SIZE; p++)
  {
...
bool ModAnalyse09::checkInertiaMoment08() 
{
  Player *player,*oldplayer;
  for (int p=START_PL; p<TEAM_SIZE*2; p++) 
  {
...
bool
ModAnalyse09::checkForAlreadyAssignedTypes()
{
  Player *player;
  ...
  for (int p=TEAM_SIZE; p<2*TEAM_SIZE; p++) 
  {
    ...
    if ( player->type >= 0 )
    {
      for (int o=TEAM_SIZE; o<2*TEAM_SIZE; o++) 
        ...
        {
          //cout<<"ModAnalyse09: *** Opp player #"<<o+1<<" cannot be of type "
          //          <<player->type<<" (single use), as opp player #"<<p+1<<" is already"<<endl;
          possTypes[o][player->type] += SINGLE_PLAYER_USAGE_WEIGHT;  
          LOG_FLD(0,<<"ModAnalyse09: *** Opp player #"<<o+1<<" cannot be of type "
...
int 
ModAnalyse09::getNumberOfRemainingTypesForPlayer( int plIdx )
{
  int returnValue = 0;
  if ( plIdx < TEAM_SIZE )
  {
...
bool 
ModAnalyse09::checkPlayerDecay09()
{
  for (int playerIndex = START_PL; playerIndex < 2*TEAM_SIZE; playerIndex++) 
  {
    if ( mayCollide[playerIndex] == true )
    {
      LOG_FLD(0,<<"ModAnalyse09: === checkPlayerDecay09 === ABORTED for player "<<playerIndex
        <<" because of COLLISION.");
      continue;
    }
    
    LOG_FLD(0,<<"ModAnalyse09: === checkPlayerDecay09 ===");
    ...
    if (playerType != -1) 
    {
      ...
      {
        LOG_FLD(0,<<"ModAnalyse09: *** Own player #"<<playerIndex+1<<" is presumably of type "
                  <<playerType<<" (player decay 09, direct assignment) ");
      }
      else
      {
        LOG_FLD(0,<<"ModAnalyse09: *** Opponent player #"<<playerIndex-TEAM_SIZE+1<<" is presumably of type "
                  <<playerType<<" (player decay 09, direct assignment) ");
      } 
    }
  }
...
void 
ModAnalyse09::showPossTypeTable()
{
  for (int playerIndex = START_PL; playerIndex < 2*TEAM_SIZE; playerIndex++) 
  {
...
const char ModAnalyse09::modName[]="ModAnalyse09";
