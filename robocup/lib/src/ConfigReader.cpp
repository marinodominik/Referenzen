...
bool ConfigReader::replace_config_file (const char* name, bool recursive) throw (std::bad_alloc) {
  std::string prefix = "";
  return file_input (".bak", name, recursive, prefix);
}

bool ConfigReader::append_from_file (const char* name, bool recursive) throw (std::bad_alloc) {
  std::string prefix = "";
  return file_input (NULL, name, recursive, prefix);
}

bool ConfigReader::file_input (const char* baksuffix, const char* name, bool recursive, std::string& prefix) throw (std::bad_alloc) {
  ...
  if (replace_mode) {
    ...
    filesuccess &= !src->fail();//((*src)!=NULL); //TG17: compilability for C++11
    filesuccess &= !src->fail();//((*dest)!=NULL); //TG17: compilability for C++11
  } else {
  ...
  while (!(src->eof())) {
    ...
    if (line[0]=='+') {
      ...
      if (recursive) {
        ...
        success &= file_input ((replace_mode ? baksuffix : NULL), fname.c_str(), true, (pos0==1 ? empty_string : prefix));
      }
      if (replace_mode)
        (*dest) << oline << '\n';
      continue;
